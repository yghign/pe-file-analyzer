...
class PEAnalyzerGUI:
    ...
    def _export_text_report(self, file_path: str):
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            ...
            if self.file_path:
                ...
                f.write(f"文件大小: {os.path.getsize(self.file_path):,} 字节\n")
                f.write(f"分析时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")

            # 各模块分析结果
            self._write_text_sections(f)
            self._write_text_imports(f)
            self._write_text_strings(f)
            self._write_text_security(f)
    ...
    """)
            ...
            f.write(f"<h1>PE文件分析报告</h1>\n")
            f.write(f"<p><strong>分析文件:</strong> {self.file_path or '未知'}</p>\n")
            f.write(f"<p><strong>分析时间:</strong> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>\n")

            # 各模块分析结果
            self._write_html_sections(f)
            self._write_html_imports(f)
            self._write_html_strings(f)
            self._write_html_security(f)
    ...
    def _export_json_report(self, file_path: str):
        ...
        report_data = {
            "file_info": {
                "path": self.file_path,
                "size": os.path.getsize(self.file_path) if self.file_path else 0,
    ...
    def _write_text_sections(self, f):
        """写入节表分析到文本报告"""
        f.write("=" * 70 + "\n")
        f.write("节表分析\n")
        f.write("=" * 70 + "\n\n")
        ...
        if 'sections' in self.analysis_results:
            ...
            if isinstance(sections_data, dict) and sections_data.get('状态') == 'success':
                sections = sections_data.get('节信息', [])
                f.write(f"总节数: {len(sections)}\n\n")

                for i, section in enumerate(sections, 1):
                    f.write(f"[节 {i}]: {section.get('名称', '未知')}\n")
                    f.write(f"  用途: {section.get('用途', '未知')}\n")
                    f.write(f"  权限: {section.get('安全特性', {}).get('权限', '未知')}\n")
                    f.write(f"  风险等级: {section.get('安全特性', {}).get('风险等级', '未知')}\n")

                    # 显示警告
                    security_analysis = section.get('安全特性', {}).get('安全分析', '')
                    if '代码节可写' in security_analysis:
                        f.write(f"  ⚠ 警告: 代码节可写，可能用于自我修改或Shellcode！\n")

                    f.write("\n")

    def _write_text_imports(self, f):
        """写入导入表分析到文本报告"""
        f.write("=" * 70 + "\n")
        f.write("导入表分析\n")
        f.write("=" * 70 + "\n\n")
        ...
        if 'imports' in self.analysis_results:
            ...
            if isinstance(imports_data, dict) and imports_data.get('status') == 'success':
                imports_list = imports_data.get('imports', [])
                f.write(f"导入DLL数量: {len(imports_list)}\n\n")
                ...
                # 每个DLL的详细信息
                for dll_info in imports_list[:10]:  # 只显示前10个DLL
                    f.write(f"[{dll_info.get('dll_name', '未知')}]\n")
                    f.write(f"  函数数量: {dll_info.get('function_count', 0)}\n")
                    f.write(f"  风险等级: {dll_info.get('risk_level', '未知')}\n")

                    # 显示可疑函数
                    suspicious_funcs = []
                    for func in dll_info.get('functions', []):
                        if func.get('suspicious', False):
                            suspicious_funcs.append(func.get('name'))

                    if suspicious_funcs:
                        f.write(f"  可疑函数: {', '.join(suspicious_funcs[:5])}\n")

                    f.write("\n")

    def _write_text_strings(self, f):
        """写入字符串分析到文本报告"""
        f.write("=" * 70 + "\n")
        f.write("字符串提取\n")
        f.write("=" * 70 + "\n\n")
        ...
        if 'strings' in self.analysis_results:
            ...
            if isinstance(strings_data, dict) and strings_data.get('status') == 'success':
                strings_count = strings_data.get('file_info', {}).get('total_strings_found', 0)
                f.write(f"提取字符串数量: {strings_count}\n\n")

                # 显示分类统计
                statistics = strings_data.get('statistics', {})
                ...
                # 显示可疑模式
                suspicious_patterns = statistics.get('suspicious_patterns', [])
    ...
    def _write_text_security(self, f):
        """写入安全分析到文本报告"""
        f.write("=" * 70 + "\n")
        f.write("安全分析总结\n")
        f.write("=" * 70 + "\n\n")
        ...
        if 'sections' in self.analysis_results:
            ...
            if isinstance(sections_data, dict) and sections_data.get('状态') == 'success':
                ...
                for section in sections:
                    security_analysis = section.get('安全特性', {}).get('安全分析', '')
                    if '代码节可写' in security_analysis:
                        all_warnings.append(f"节 '{section.get('名称')}' 可写代码节，可能存在安全风险")
